...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO emerald_prices (Gemstone, Weight, Currency, Price) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssd", $Gemstone, $Weight, $Currency, $Price);

    if ($stmt->execute()) {
        // ✅ Redirect to view page after successful insert
        header("Location: view_emerald.php?success=1");
        exit();
    } else {
    ...
    <form method="POST" action="">
      ...
      <div class="form-group">
        ...
        <select name="Gemstone" required>
          ...
          <option value="Bluish Green Emerald">Bluish Green Emerald</option>
          <option value="Green Emerald">Green Emerald</option>
          <option value="Lab Grown Emerald">Lab Grown Emerald</option>
          <option value="Yellowish Green Emerald">Yellowish Green Emerald</option>
        </select>
      </div>
       ...
       <div class="form-group">
        ...
        <select name="Weight" required>
          ...
          <option value="5 carat">5 carat</option>
        </select>
      </div>
      ...
      <button type="submit">💰 Save Emerald Rate</button>
    </form>
  </div>
</div>
