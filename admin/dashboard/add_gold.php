...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($currency_id && $purity && $price > 0) {
        $sql = "INSERT INTO gold_prices (currency_id, Purity, Prices, created_at, updated_at) 
                VALUES (?, ?, ?, NOW(), NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isd", $currency_id, $purity, $price);

        if ($stmt->execute()) {
            header("Location: view_gold.php?success=1");
            exit();
        } else {
            echo "<p style='color:red; text-align:center;'>❌ Error: " . $stmt->error . "</p>";
        }
    } else {
        echo "<p style='color:red; text-align:center;'>❌ Invalid input. Price must be greater than 0.</p>";
    }
}
?>
