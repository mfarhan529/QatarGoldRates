...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO ruby_prices (Gemstone, Weight, Currency, Price) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssd", $Gemstone, $Weight, $Currency, $Price);

    if ($stmt->execute()) {
        // ✅ Redirect to view page after successful insert
        header("Location: view_ruby.php?success=1");
        exit();
    } else {
    ...
    <form method="POST" action="">
      ...
      <div class="form-group">
        ...
        <select name="Gemstone" required>
          ...
          <option value="Natural Ruby">Natural Ruby</option>
        </select>
      </div>
       ...
       <div class="form-group">
        ...
        <select name="Weight" required>
          ...
          <option value="10 carat">10 carat</option>
        </select>
      </div>
      ...
      <button type="submit">💰 Save Ruby Rate</button>
    </form>
  </div>
</div>
