...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $Price_Range       = $_POST['Price-Range'] ?? null;

    if ($currency_id &&  $weight_id && $type && $Price_Range) {
        echo $sql = "INSERT INTO diamond_prices (currency_id, weight_id, type, Price_Range, created_at) 
                VALUES (?, ?, ?, ?, NOW())"; die;
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("iiss", $currency_id, $weight_id, $type, $Price_Range);


        if ($stmt->execute()) {
            header("Location: view_diamond.php?success=1");
            exit();
        } else {
            echo "<p style='color:red; text-align:center;'>❌ Error: " . $stmt->error . "</p>";
        }
    } else {
        echo "<p style='color:red; text-align:center;'>❌ Invalid input. All fields are required and price must be.</p>";
    }
}
?>
<!DOCTYPE html>
    ...
    <form method="POST" action="">
      ...
      <div class="form-group">
        ...
        <select name="type" required>
          ...
          <option value="Fancy Colour Diamond">Fancy Colour Diamond</option>
          <option value="Lab Grown Diamond">Lab Grown Diamond</option>
          <option value="Natural Diamond">Natural Diamond</option>
          <option value="Treated Diamond">Treated Diamond</option>
