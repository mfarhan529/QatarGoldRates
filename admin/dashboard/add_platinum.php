...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO platinum_prices ( Weight, Currency, Price) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssd", $Weight, $Currency, $Price);

    if ($stmt->execute()) {
        // ✅ Redirect to view page after successful insert
        header("Location: view_platinum.php?success=1");
        exit();
    } else {
    ...
    <form method="POST" action="">
       ...
       <div class="form-group">
        ...
        <select name="Weight" required>
          ...
          <option value="2 Gram">1 Gram</option>
      ...
      <button type="submit">💰 Save Platinum Rate</button>
    </form>
  </div>
</div>
